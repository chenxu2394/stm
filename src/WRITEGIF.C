...
void GIFEncode ( FILE* fp, int GWidth, int GHeight, int GInterlace, 
    int Background, int BitsPerPixel, unsigned char *pal, 
    ifunptr GetPixel );
     ...
     ifunptr GetPixel )
{
        ...
        hsize=HSIZE;
        Width=Height=curx=cury=Interlace=n_bits=maxcode=offset=g_init_bits=
            ClearCode=EOFCode=cur_accum=cur_bits=a_count=0;
        for(i=0;i<256;i++) accum[i]=0;    
