...
//               auto_z_below(IN_ZERO - Z_SAFETY * 10);
		...
		case PRESCAN_SCAN_START:
//        SendMessage(handle,WM_COMMAND,SCAN_START,0);
          scan_now = 1;
          EndDialog(hDlg, TRUE);
          return (TRUE);
          break;
...
void repaint_inter_step(HWND hDlg)
{
    ...
    sprintf(string,"%0.2f",20.0 / 4096.0 * scan_x_gain * A_PER_V / SD->inter_step_delay * 1000000);
    /* 20.0 should be changed for +-5 Volts! */
