...
int dep_pulse_z_after=DEP_PULSE_Z_AFTER_AUTO;
...
BOOL FAR PASCAL DepDlg(HWND hDlg,unsigned Message,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(Message) {
    case WM_INITDIALOG:
        ...
        lines_max_i_set=calc_i_set(lines_max_i_setpoint,i_set_range,tip_gain);
        move_i_set=calc_i_set(move_i_setpoint,i_set_range,tip_gain);
        write_i_set=calc_i_set(write_i_setpoint,i_set_range,tip_gain);
...
*/
	...
	case DEP_V_SCALE_MAX_SCROLL:
	    if( dep_current_channel == i_in_ch) {
                ...
                minid=dep_vert_min[i_in_ch]+abs(dep_vert_max_max-dep_vert_min_min)/100;
                maxid=dep_vert_max_max;
                deltd=abs(dep_vert_max_max-dep_vert_min_min)/100;
                ddeltd=abs(dep_vert_max_max-dep_vert_min_min)/10;
	    }
	    else {
                d = (double) dep_vert_max[zi_ch];
                minid = dep_vert_min[zi_ch] + abs(dep_vert_max_max - dep_vert_min_min) / 100;
                maxid = dep_vert_max_max;
                deltd = abs(dep_vert_max_max - dep_vert_min_min) / 100;
                ddeltd = abs(dep_vert_max_max - dep_vert_min_min) / 10;
	    }
	...
	case DEP_V_SCALE_MIN_SCROLL:
	    if( dep_current_channel == i_in_ch) {
                ...
                maxid=dep_vert_max[i_in_ch]-abs(dep_vert_max_max-dep_vert_min_min)/100;
                deltd=abs(dep_vert_max_max-dep_vert_min_min)/100;
                ddeltd=abs(dep_vert_max_max-dep_vert_min_min)/10;
	    }
	    else {
                ...
                minid = dep_vert_min_min;
                maxid = dep_vert_max[zi_ch] - abs(dep_vert_max_max - dep_vert_min_min) / 100;
                deltd = abs(dep_vert_max_max - dep_vert_min_min) / 100;
                ddeltd = abs(dep_vert_max_max - dep_vert_min_min) / 10;
	    }
	   break;
	...
	case DEP_V_SCALE_MAX_SCROLL:
	    if( dep_current_channel == i_in_ch) {
                dep_vert_max[i_in_ch] = d;
	    }
	    else {
                dep_vert_max[zi_ch] = d;
	    }
            repaint_vert_scale(hDlg);
	...
	case DEP_V_SCALE_MIN_SCROLL:
	    if( dep_current_channel == i_in_ch) {
                dep_vert_min[i_in_ch] = d;
	    }
	    else {
                dep_vert_min[zi_ch] = d;
	    }
            repaint_vert_scale_min(hDlg);
	    repaint_oscill_data(hDlg, dep_current_channel);
	    break;
...
//                            strcpy(data2->dep_filename,current_file_dep);
	    ...
	    case DEP_V_SCALE_MAX:
                if (!out_smart)
                {
                    ...
                    if ( dep_current_channel == i_in_ch) {
                      dep_vert_max[i_in_ch] = in_vtod(atof(string));
                      dep_vert_max[i_in_ch] = min(max(dep_vert_max[i_in_ch],
                         dep_vert_min[i_in_ch]+abs(dep_vert_max_max-dep_vert_min_min)/100),
                         dep_vert_max_max);
		      ...
		      dep_vert_max[zi_ch] = in_vtod(atof(string));
                      dep_vert_max[zi_ch] = min(max(dep_vert_max[zi_ch],
                         dep_vert_min[zi_ch] + abs(dep_vert_max_max - dep_vert_min_min) / 100),
                         dep_vert_max_max);
                      SetScrollPos(GetDlgItem(hDlg, DEP_V_SCALE_MAX_SCROLL), 
	    ...
	    case DEP_V_SCALE_MIN:
                if (!out_smart)
                {
                    ...
                    if (dep_current_channel == i_in_ch) {
		      dep_vert_min[i_in_ch] = in_vtod(atof(string));
                      dep_vert_min[i_in_ch] = min(max(dep_vert_min[i_in_ch],dep_vert_min_min),
                         dep_vert_max[i_in_ch]-abs(dep_vert_max_max-dep_vert_min_min)/100);
		      ...
		      dep_vert_min[zi_ch] = in_vtod(atof(string));
                      dep_vert_min[zi_ch] = min(max(dep_vert_min[zi_ch],dep_vert_min_min),
                         dep_vert_max[zi_ch]-abs(dep_vert_max_max-dep_vert_min_min)/100);
                      SetScrollPos(GetDlgItem(hDlg, DEP_V_SCALE_MIN_SCROLL), 
...
*/
				...
				//pushkin
				//autominus if the box is cheched
				/*
				time( &start );
				while ( finish - start < dep_delay )  time( &finish ); //wait for the serial port
				if(dep_pulse_z_after == DEP_PULSE_Z_AFTER_AUTO) auto_z_below(IN_ZERO);
				*/
        ...
        switch(LOWORD(wParam)) {
            ...
            case DEP_PULSE_PERCENT_CHANGE:
                if(!out_smart)
                {
                    ...
                    dep_pulse_percent_change = atof(string); 
/*
...
*/
            ...
            case DEP_PULSE_BIAS_SPEED:
                if(!out_smart) {
                    ...
                    dep_pulse_bias_speed = min(max(DEP_PULSE_BIAS_SPEED_MIN,
...
static void repaint_pulse_number(HWND hDlg)
{
        ...
        SetScrollPos(GetDlgItem(hDlg,DEP_PULSE_NUMBER_SCROLL),
            SB_CTL,dep_pulse_number,TRUE);
        out_smart=0;
}
...
{
        out_smart=1;
    SetDlgItemText(hDlg, DEP_WRI_BIAS, gcvt(dtov_bias(write_bias,sample_bias_range), 5, string));
    SetScrollPos(GetDlgItem(hDlg,DEP_WRI_BIAS_SC),SB_CTL,write_bias,TRUE);
        out_smart=0;
...
void dep_pulse_dsp(HWND hDlg)
{
	...
	if(dep_pulse_measure_i) flags+=0x02; 
	...
	//do not receive data back from the DSP if we didn't read anything
	if((dep_pulse_measure_i) | (dep_pulse_measure_z)){
	  ...
	  for(i=0; i<pts_number; i++){
		  ...
		  *(dep_oscill_data[channel]->data2d + i)= read_2_serial();
	  }
	}	
...
void dep_pulse3(HWND hDlg)
{
    ...
    dep_pulse_this_duration=dep_pulse_duration;
    dep_data_max=(dep_measure_extra*2+dep_pulse_duration)/
        ADC_DELAY/dep_pulse_avg_pts+1;
...
/* version 5 */    
    ...
    this_data->dep_pulse_z_offset_overshoot_percentage = 
        dep_pulse_z_offset_overshoot_percentage;
...
void repaint_oscill_data(HWND hDlg, int ch)
{
	...
	// move to starting position
        xs = x1 + (*((dep_oscill_data[ch])->time2d) - start_time) *
		(x2 - x1 + 1) / total_time;
...
//        MoveTo(dep_oscill_hdc, xs, ys);
	    ...
	    // draw a line to the next data position
            xs = x1 + (*((dep_oscill_data[ch])->time2d + j) - start_time) *
		    (x2 - x1 + 1) / total_time;
...
*/
        ...
        if (dep_num_lines>1)
        {
            ...
            current_i_set=write_i_set*pow(lines_max_i_set/write_i_set,
                ((double)i)/((double)(dep_num_lines-1)));
            current_write_time=write_time*pow((double)max_write_time/(double)write_time,
                ((double)i)/((double)(dep_num_lines-1)));
/*
