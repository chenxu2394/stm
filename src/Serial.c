...
void scan_setup(int protection_type,float limit_percent,int interdelay,int readdelay)
{
	...
	unsigned int conv_delay_read = convert_serial_delay(readdelay);
				...
				int move_delay,int read_delay)
{
  ...
  unsigned int serial_read_delay = convert_serial_delay(read_delay);
				  ...
				  int move_delay,int read_delay)
{
  ...
  unsigned int lm_read_delay = convert_serial_delay(read_delay);
			   ...
			   double chng_ratio, int aver_every, int flags)
{
	...
	out_buff[20] = (char)(aver_pts & 0x0FF);

	out_buff[21] = (char)((ignore_init & 0xFF00) >> 8);
	out_buff[22] = (char)(ignore_init & 0x0FF);

	out_buff[23] = (char)((chng_t & 0xFF00) >> 8);
	out_buff[24] = (char)(chng_t & 0x0FF);
	out_buff[25] = (char)((after_t & 0xFF00) >> 8);
	out_buff[26] = (char)(after_t & 0x0FF);

	out_buff[27] = (char)((((int)(chng_ratio*32)) & 0xFF00) >> 8);
	out_buff[28] = (char)(((int)(chng_ratio*32)) & 0x0FF);
